...
public abstract class BeanTemplate<T extends ModelTemplate, S extends ServiceTemplate> implements Serializable {
    ...
    public T getItem() {
        if (item == null) {
            item = (T) getService().getNewItemInstance();
        }
        return item;
    }
    ...
    public void removeItem(T item) {
        try {
            if (getService().getDao().deleteItem(item)) {
                handleInfo("OK", "Eintrag entfernt.");
            }
        } catch (Exception e) {
            handleException("Fehler", e);
        }
    ...
    public void componentAjaxUpdate(String s) {
        if (s != null && !s.isBlank()) {
            PrimeFaces.current().ajax().update(s);
        }
    }

    public void updateContentForm() {
        componentAjaxUpdate(":contentForm");
    }

    public void updateMessageBox() {
        componentAjaxUpdate(":messageBox");
    }
